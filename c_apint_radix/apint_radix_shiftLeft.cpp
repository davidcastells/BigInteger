...
{
    big_integer_apint_radix_shiftLeft(r, r, bits );
}
...
void big_integer_apint_radix_shiftLeft(limbs_radix_array r, limbs_radix_array a, int sv)
{
    ...
    for (int i= NUM_BIG_INTEGER_APINT_RADIX_LIMBS-1; i >= 0; i--)
    {
        ...
        if ((i-limbsShifted) >= 0 && (i-limbsShifted) < NUM_BIG_INTEGER_APINT_RADIX_LIMBS)
            v1 = a[i-limbsShifted] << limbBitsShifted;
        if ((i-limbsShifted-1) >= 0 && (i-limbsShifted-1) < NUM_BIG_INTEGER_APINT_RADIX_LIMBS && cs)

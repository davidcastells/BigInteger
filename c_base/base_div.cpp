 ...
 * @param nr
 */
void big_integer_base_div_naive(unsigned int* x_data, const unsigned int x_base, const unsigned int x_size,
...
{
    if (base_verbosity> 6) cout << "big_integer_div_naive x = " << big_integer_base_toHexString(x_data, x_base, x_size) << endl;
    if (base_verbosity> 6) cout << "big_integer_div_naive /   " << big_integer_base_toHexString(y_data, y_base, y_size) << endl;
    
    big_integer_base_zero(q_data, q_base, q_size);
...
//        big_integer_copy(nq_data, nq_base, nq_size, q_data, q_base, q_size);
    ...
    else
    {
		...
		big_integer_base_range(r_data, r_base, r_size, x_data, x_base, x_size, rl-1, downBit);
                ...
                if (base_verbosity> 6)
                    cout << "big_integer_div_naive x[" << (rl-1) << ","<< downBit <<" ]   " << big_integer_base_toHexString(r_data, r_base, r_size) << std::endl;
		...
		big_integer_base_inc(q_data, q_base, q_size);
		big_integer_base_subtract_short(r_data, r_base, r_size, y_data, y_base, y_size);	
	
                if (base_verbosity> 6) std::cout << "big_integer_div_naive q=" << big_integer_base_toHexString(q_data, q_base, q_size) << std::endl;
		...
		while (downBit >= 0)
		{
			...
			if (ret)
			{
				big_integer_base_shiftLeft_short(q_data, q_base, q_size, 1);	// put a zero in q
			}
			else
			{
				big_integer_base_shiftLeft_short(q_data, q_base, q_size, 1);	// put a one in q
				big_integer_base_inc(q_data, q_base, q_size);
